...
/**
 * <p>create row on table serial search.</p>
 * @param data
 */
...
/**
 * <p>create table search serial.</p>
 * @param i
 * @param max_elem
 */
function createTable(i, max_elem) {
	...
	if(members !=null && members!= ""){
		for ( var rowIdx = i; rowIdx < max_elem; rowIdx++) {
			...
			strRow += "<td>" + jsonTR.serial + "</td><td>" + jsonTR.date + "</td><td>" + jsonTR.purchase
					+ "</td>";
		...
		strRow += "<td class = bntdelete>" + "" + "</td><td class = bntdelete>" + "" + "</td><td class = bntdelete>" + ""
		+ "</td>";
...
/**
 * <p>function load data display list serial.</p>
 */
function formLoad() {
	$.get('searchserial?serial=' + params.serial + '&date=' + params.date + '&purchase=' + params.purchase + '',
			function(data) {
				if(data!=null && data!=""){
					createRow(data);
				}else{
...
/**
 * <p>call page edit serial.</p>
 * @param rowIdx
 */
function reply_click(rowIdx) {
	window.document.location.href = "/BundleBoxService/webapps/myapp/html/serialmanager/editserial.html?serial="
